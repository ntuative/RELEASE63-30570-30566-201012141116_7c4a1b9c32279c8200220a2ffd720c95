package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1241:String = "asset-libraries";
      
      private static const const_1249:String = "library";
      
      private static const const_1245:String = "service-libraries";
      
      private static const const_1247:String = "library";
      
      private static const const_1248:String = "component-libraries";
      
      private static const const_1243:String = "library";
      
      private static const const_1244:String = "error_data";
      
      private static const const_1246:String = "error_cat";
      
      private static const const_1242:String = "error_desc";
       
      
      private var var_339:LibraryLoaderQueue;
      
      private var var_745:IEventDispatcher;
      
      private var var_497:uint;
      
      private var var_496:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         this.var_498 = new Array();
         this.var_610 = new Array();
         var_414 = param1;
         var _loc3_:int = 0;
         while(_loc3_ < CoreComponentContext.const_140)
         {
            this.var_498.push(new Array());
            this.var_610.push(0);
            _loc3_++;
         }
         this.var_1822 = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_414.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_948:
               debug("Core; using simple frame update handler");
               this.var_496 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_810:
               debug("Core; using complex frame update handler");
               this.var_496 = this.complexFrameUpdateHandler;
               break;
            case Core.const_1299:
               debug("Core; using profiler frame update handler");
               this.var_496 = this.profilerFrameUpdateHandler;
               this.var_249 = new Profiler(this);
               attachComponent(this.var_249,[new IIDProfiler()]);
               var_414.addChild(new ProfilerViewer(this.var_249));
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_496 = this.debugFrameUpdateHandler;
         }
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            var_414.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
            var_414 = null;
            _loc3_ = 0;
            while(_loc3_ < CoreComponentContext.const_140)
            {
               _loc2_ = this.var_498[_loc3_] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1244,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1246,String(param3));
         ErrorReportStorage.setParameter(const_1242,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_745 = loadingEventDelegate;
         if(this.var_339 == null)
         {
            this.var_339 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1241)[0];
            if(node != null)
            {
               list = node.child(const_1249);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_294,e);
         }
         try
         {
            node = config.child(const_1245)[0];
            if(node != null)
            {
               list = node.child(const_1247);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_294,e);
         }
         try
         {
            node = config.child(const_1248)[0];
            if(node != null)
            {
               list = node.child(const_1243);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_294,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1466);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_745)
         {
            this.var_745.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_745)
         {
            ...
            this.var_745.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_745)
                  {
                     this.var_745.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_497 - this.var_339.length,this.var_497,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:Array = this.var_498[param2] as Array;
         _loc3_.push(param1);
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         {
            _loc3_ = this.var_498[_loc4_] as Array;
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_496(_loc2_,_loc2_ - this.var_1822);
         this.var_1822 = _loc2_;
      }
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(criticality < CoreComponentContext.const_140)
         {
            ...
            receivers = this.var_498[criticality];
            index = 0;
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var msCurrentTime:uint = param1;
         var msSinceLastUpdate:uint = param2;
         var maxTimePerFrame:uint = 1000 / var_414.stage.frameRate;
         criticality = 0;
         while(criticality < CoreComponentContext.const_140)
         {
            t = getTimer();
