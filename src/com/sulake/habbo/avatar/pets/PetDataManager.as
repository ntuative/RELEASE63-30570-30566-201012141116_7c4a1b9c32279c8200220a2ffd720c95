package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      private var var_134:AvatarRenderManager;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_362.getValue(_loc4_) == null)
               {
                  this.var_362.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         if(param1 == null || this.var_445 == null)
         {
            return;
         }
         var _loc2_:int = this.var_445.indexOf(param1);
         if(_loc2_ != -1)
         {
            this.var_445.splice(_loc2_,1);
         }
      }
      ...
      {
         var _loc1_:* = null;
         while(this.var_445.length > 0)
         {
            _loc1_ = this.var_445.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_362.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      {
         var _loc4_:PetData = this.var_362.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_362.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_362.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.const_1199.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         if(param2 == AvatarScaleType.const_80)
         {
            _loc5_ = 0;
         ...
         {
            if(this.var_1232.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_530.getValue(_loc6_) == null)
               {
                  ...
                  this.var_530.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_445.indexOf(param3) == -1)
                  {
                     this.var_445.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_530.getKeys())
         {
            _loc5_ = this.var_530.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
