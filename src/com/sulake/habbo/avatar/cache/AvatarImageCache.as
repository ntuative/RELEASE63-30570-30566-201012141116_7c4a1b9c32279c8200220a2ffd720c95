package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1568:int = 60000;
      ...
      private var var_314:IActiveActionData;
      
      private var var_367:AvatarImageBodyPartContainer;
      
      private var var_1306:String;
      
      private var var_107:AvatarImagePartContainer;
      
      private var var_2355:Point;
      ...
      private var var_378:BitmapDataAsset;
      
      private var var_855:BitmapData;
      
      private var var_2356:ByteArray;
      
      private var var_2357:ImageData;
      
      private var var_1667:PaletteMap;
      ...
      public function dispose() : void
      {
         ...
         {
            _loc2_ = this.var_232.getKeys();
            for each(_loc3_ in _loc2_)
            {
               _loc1_ = this.var_232.getValue(_loc3_) as AvatarImageBodyPartCache;
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
               }
      ...
      public function disposeInactiveActions(param1:int = 60000) : void
      {
         ...
         var _loc4_:Array = this.var_232.getKeys();
         for each(_loc5_ in _loc4_)
         {
            _loc3_ = this.var_232.getValue(_loc5_) as AvatarImageBodyPartCache;
            if(_loc3_ != null)
            {
      ...
      public function resetBodyPartCache(param1:IActiveActionData) : void
      {
         var _loc2_:* = null;
         var _loc4_:* = null;
         var _loc3_:Array = this.var_232.getKeys();
         for each(_loc4_ in _loc3_)
         {
            _loc2_ = this.var_232.getValue(_loc4_) as AvatarImageBodyPartCache;
            if(_loc2_ != null)
            {
      ...
      {
         if(this.var_688 == param1)
         {
            return;
         }
         if(this.var_688 == GeometryType.const_922 && param1 == GeometryType.const_935 || this.var_688 == GeometryType.const_935 && param1 == GeometryType.const_922)
         {
            this.var_688 = param1;
            this.var_560 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         if(this._bodyPartCache == null)
         {
            ...
            this.var_232.add(param1,this._bodyPartCache);
         }
         var _loc3_:int = this._bodyPartCache.getDirection();
         var _loc4_:int = param2;
         this.var_314 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_314;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_314 || !this.var_314.definition))
         {
            if(this.var_314.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_314.definition.state + "." + this.var_314.actionParameter);
               _loc13_ = param2 - this.var_314.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_314.overridingAction);
                  if(_loc14_ != null)
                  {
                     ...
                     {
                        this.var_314 = _loc14_.action;
                     }
                     if(_loc14_.type == AnimationLayerData.const_812)
                     {
                        if(_loc14_.action != null)
                        {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_523)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1360.getFigure(),_loc5_,this.var_688,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_367 = _loc10_.getImageContainer(_loc4_);
         if(this.var_367 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_367 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_314);
            if(this.var_367 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_367,_loc4_);
         }
         this.var_367.offset = _loc8_;
         return this.var_367;
      }
      
      public function getBodyPartCache(param1:String) : AvatarImageBodyPartCache
      {
         var _loc2_:AvatarImageBodyPartCache = this.var_232.getValue(param1) as AvatarImageBodyPartCache;
         if(_loc2_ == null)
         {
            _loc2_ = new AvatarImageBodyPartCache();
            this.var_232.add(param1,_loc2_);
         }
         return _loc2_;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_560)
         {
            this.var_560 = this._structure.getCanvas(this._scale,this.var_688);
            if(!this.var_560)
            {
               return null;
            }
            this.var_2355 = new Point(0,this.var_560.height - int(this.var_560.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1262[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_704 || _loc10_ == AvatarAction.const_336;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1306 = this.var_107.bodyPartId;
               this.var_165 = this.var_107.partType;
               this.var_1666 = this.var_107.partId;
               ...
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_107.flippedPartType != this.var_165)
                     {
                        this.var_165 = this.var_107.flippedPartType;
                     }
                  }
               }
               this.var_1305 = this._scale + "_" + this.var_107.action.assetPartDefinition + "_" + this.var_165 + "_" + this.var_1666 + "_" + _loc5_ + "_" + this._frame;
               this.var_378 = this._assets.getAssetByName(this.var_1305) as BitmapDataAsset;
               if(this.var_378 == null)
               {
                  this.var_1305 = this._scale + "_std_" + this.var_165 + "_" + this.var_1666 + "_" + _loc5_ + "_0";
                  this.var_378 = this._assets.getAssetByName(this.var_1305) as BitmapDataAsset;
               }
               if(!(!this.var_378 || !this.var_378.content))
               {
                  if(this.var_107.paletteMapId > -1)
                  {
                     this.var_1667 = this._structure.renderManager.paletteManager.getPalette(this.var_107.paletteMapId);
                     if(this.var_1667 != null)
                     {
                        this.var_855 = this.var_378.content as BitmapData;
                        this.var_166 = new BitmapData(this.var_855.width,this.var_855.height);
                        this.var_2356 = this.var_1667.colorize(this.var_855.getPixels(this.var_855.rect));
                        this.var_166.setPixels(this.var_166.rect,this.var_2356);
                     }
                     else
                     {
                        this.var_166 = (this.var_378.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1306 == AvatarBodyPartType.const_200 || this.var_1306 == AvatarBodyPartType.const_1352 || this.var_1306 == AvatarBodyPartType.const_1265) && this.var_107.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_166.colorTransform(this.var_166.rect,this.var_107.color.colorTransform);
                  }
                  if(this.var_107.isBlendable)
                  {
                     this.var_166.colorTransform(this.var_166.rect,this.var_107.blendTransform);
                  }
                  this.var_854 = this.var_378.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_48)
                     {
                  ...
                  this.var_2357 = new ImageData(this.var_166,this.var_854,_loc7_);
                  this.var_853.push(this.var_2357);
               }
            }
            _loc8_--;
         }
         ...
         var _loc13_:ImageData = this.createUnionImage(this.var_853,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2355.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_48)
            {
         ...
         _loc8_ = this.var_853.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_853.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
