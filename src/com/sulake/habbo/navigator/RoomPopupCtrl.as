package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2165 = new RoomThumbnailRenderer(param1);
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_156 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_156.visible = true;
         Util.hideChildren(this.var_156);
         this.refreshRoomName(this.var_156,this.var_102);
         if(this.var_102.ownerName != "" && this.var_102.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_156,!!this.var_102.event ? this.getEventCtg(this.var_102.categoryId) : this.getRoomCtg(this.var_102.categoryId));
         this.refreshRoomDesc(this.var_156,this.var_102);
         this.refreshExtraCont();
         this.refreshInfo(this.var_156,"trading_allowed",this.var_102.allowTrading);
         this.refreshInfo(this.var_156,"doormode_doorbell",this.var_102.doorMode == 1);
         this.refreshInfo(this.var_156,"doormode_password",this.var_102.doorMode == 2);
         Util.moveChildrenToColumn(this.var_156,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_156.height = Util.getLowestPoint(this.var_156);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_156,this.var_102.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_156.findChildByName("roomowner"));
         if(this.var_102.ownerName != "" && this.var_102.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_156.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1065.refreshTags(_loc1_,this.var_102.tags);
         ...
         this.var_2165.refreshThumbnail(_loc1_,this.var_102.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_208,Tabs.const_231,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
